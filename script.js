...
function createRoom() {
    ...
    const roomData = {
        ...
        seed: seed,
        players: {
            p1: { score: 0, finished: false, ready: true },
            p2: { score: 0, finished: false, ready: false }
        }
    };
...
function joinRoom() {
    ...
    db.ref('rooms/' + roomCode).once('value', snapshot => {
        ...
        db.ref('rooms/' + roomCode).update({ status: 'in-progress' });
...
function listenToRoom() {
    roomListener = db.ref('rooms/' + roomCode).on('value', snapshot => {
        ...
        // --- GAME START TRIGGER ---
        if (room.status === 'in-progress' && document.getElementById('gameArea').classList.contains('hidden')) {
        ...
        if (!oppData && room.status === 'in-progress') {
